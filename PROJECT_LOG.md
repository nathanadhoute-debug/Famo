...
✅ Fonctionnel : landing (positionnement "Vieillir accompagné. Aider ensemble.", sans faux témoignage), auth (login/signup/confirmation email/**mot de passe oublié**), onboarding, dashboard (6 pages, design éditorial), invitations (création, email, acceptation), connexion Supabase (avec filet de sécurité en dur), **SMTP personnalisé Resend pour tous les emails auth**, **RLS actif sur families/family_members**, **favicon/icônes de lien corrects**, **suivi médicaments (ajout, horaires, ordonnance) + les 3 crons Vercel (daily-doses, rx-expiry, visit-reminder) implémentés et vérifiés fonctionnels en prod** (bug de middleware qui les bloquait silencieusement corrigé le 14/07), **heure des visites correctement conservée** (`visit_date` en timestamptz, affichage cohérent Accueil/Relais en heure de Paris), **Journal filtrable par tag**, **historique des mesures replié par défaut**, **4e cron `overdue-doses` (alerte dose non prise 1h après l'heure prévue) codé et testé fonctionnel**, **adresse contact@famo.health opérationnelle**, **page de confidentialité RGPD** (`/confidentialite`). Test mobile et test RLS avec un compte non-admin réalisés par l'utilisateur le 14/07/2026, rien à signaler.

⚠️ Pas encore fait :
- **cron-job.org pas configuré** : le cron `overdue-doses` (alerte médicament non pris) est codé, testé manuellement et fonctionnel, mais ne se déclenche pas encore tout seul — il faut créer un compte gratuit sur cron-job.org et y programmer un appel horaire vers `https://famo.health/api/cron/overdue-doses` avec le header `Authorization: Bearer <CRON_SECRET>`. Voir Phase 9.
...
## Phase 8 — Bug `visit_date` tronqué, QA préventive, Journal et positionnement (14/07/2026)

Suite directe de la Phase 7 : en vérifiant l'affichage d'une visite de test après avoir corrigé les crons, découverte que l'Accueil (rendu serveur, Node/UTC) et Relais (rendu client, navigateur Europe/Paris) affichaient deux heures différentes pour la même visite (minuit UTC formaté dans deux fuseaux).

- **`visit_date` migré de `date` à `timestamptz`** (migration `005_visit_datetime.sql`, appliquée manuellement) : le formulaire "Programmer une visite" demande une heure (`datetime-local`) mais la colonne était une `date` pure — Postgres tronquait silencieusement l'heure choisie. Fix : colonne en `timestamptz`, conversion `new Date(form.date).toISOString()` côté client avant envoi (`VisitsManager.tsx`), affichage/calage par jour désormais explicitement ancré sur `Europe/Paris` via le nouveau helper `parisDateKey()` (`lib/format.ts`). Le cron `visit-reminder` ne peut plus faire `.eq("visit_date", dateExacte)` — récupère une fenêtre large puis filtre sur le jour calendaire à Paris.
- **QA préventive** sur la même classe de bug (fuseau horaire non explicite) dans tout le reste du projet : `VitalsManager.tsx` affichait l'heure d'une mesure sans fuseau explicite (vrai risque de décalage serveur/client) — corrigé. `MedicationsManager.tsx` et `DocumentsManager.tsx` harmonisés par cohérence (risque plus faible, date seule sans heure). Vérifié qu'aucune autre route technique ne manquait au matcher du middleware (seul `api/cron` manquait, déjà fixé en Phase 7).
- **Journal** : ajout d'un filtre par tag sur le fil de notes (`JournalManager.tsx`) — les tags ne servaient jusque-là qu'à taguer une nouvelle note, aucun moyen de filtrer les notes déjà publiées.
- **`lib/status.ts`** : quand l'alerte "attention" vient d'une note taguée urgence, affiche désormais le contenu réel de la note (`« Baba est fatigué »`) au lieu du texte générique "note d'urgence dans le journal" — plus actionnable pour un aidant qui lit l'Accueil.
- **Comportement confirmé (pas un bug)** : `deriveParentStatus()` ne regarde que la **toute dernière** note du journal, pas un historique — écrire une nouvelle note le lendemain (sans besoin de la taguer urgence) suffit à faire repasser l'Accueil en mode calme automatiquement.
- **Landing — faux témoignage retiré** : la citation "On était trois enfants... — Une famille utilisatrice" était fictive (Famō n'a pas encore d'utilisateurs externes réels). Remplacée par un vrai positionnement assumé, sans attribution inventée : **"Vieillir accompagné. Aider ensemble."**, utilisé en H1 du hero et en titre de l'onglet navigateur (`metadata.title`). La section fonctionnalités garde son propre titre "Tout ce qui compte, au même endroit", pas de doublon.

## Phase 9 — Historique replié + 4e cron : dose non prise (15/07/2026)

- **Santé** : l'historique des mesures (`VitalsManager.tsx`) n'avait aucune limite d'affichage et grossissait sans fin. Replié derrière un bouton "Voir l'historique (N)" par défaut fermé ; les cartes "Mesures" (dernière valeur par type) restent affichées normalement en haut de page.
- **Nouveau cron `overdue-doses`** (`src/app/api/cron/overdue-doses/route.ts`) : alerte le cercle familial quand **n'importe quelle** dose programmée (pas seulement les médicaments marqués "critique") n'est toujours pas cochée comme prise **1h après** l'heure prévue. Mail inclut le nom du médicament, le dosage et le proche concerné.
  - Migration `006_overdue_dose_alerts.sql` : nouvelle colonne `doses.alert_sent_at` (anti-spam, une seule alerte par dose) + fonction SQL `overdue_doses(grace_minutes)`.
  - Migration `007_overdue_doses_add_dose.sql` : ajoute `m.dose` au retour de la fonction (nécessite `drop function` avant `create or replace` quand on change le type de retour — Postgres ne l'autorise pas directement).
  - **Piège SQL rencontré** : `current_time` (type `timetz`) ne se soustrait pas directement à un `time` pour comparer à un `interval`. Fix : ancrer explicitement sur `(now() at time zone 'Europe/Paris')::time`, cohérent avec la convention Paris déjà adoptée en Phase 8.
  - **Cette route n'est PAS dans `vercel.json`** : le plan Vercel Hobby limite les cron jobs à 1 exécution/jour (une fréquence plus élevée fait échouer le déploiement). Elle est appelée par un **service de cron externe gratuit (cron-job.org)**, toutes les heures, avec le header `Authorization: Bearer <CRON_SECRET>` — même secret que les 3 autres crons.
